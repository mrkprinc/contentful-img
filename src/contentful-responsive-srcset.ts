...
function createContentfulImageUrl(baseUrl: string, params: IImageParams): string {
...
function createContentfulSrcSet(
  ...
  if(!(image && baseUrl && intrinsicWidth && intrinsicHeight)) return { src: "" }
  ...
  const srcSet = Array.from(responsiveWidths)
    .map(w =>
      `${createContentfulImageUrl(baseUrl, { w, fm: format })} ${Math.round(w)}w`
    )
    .join(',\n')
...
export {
  createContentfulSrcSet,
  createContentfulImageUrl,
}
